module.exports = function toReadable (number) {
  ...
  let digit = number%10;
  if (number < 100) {
      return tens[Math.trunc(number/10)-2]+(digit > 0? " " + num[digit]: "");
  }
  if (number < 1000) {
      return num[Math.trunc(number/100)] + " hundred" + (number%100 === 0? "": " " + toReadable(number%100));
  }
  return num[Math.trunc(number/1000)] + " thousand" + (number%1000 != 0? " " + toReadable(number%100): "");
